package App::Yath::Plugin::YathUIDB::v2;
...
use GetOpt::Yath; # For the "option*" prototypes
use Test2::Harness::UI::Util qw/config_from_settings/;
use Test2::Harness::UI::UUID;
...
option_group {prefix => 'yathui', category => "YathUI Options"} => sub {
    ...
    option resources => (
        type => 'Count', # XXX Not sure if this is right, as the remap wasn't obvious to me here.
    ...
    option_post_process( 200, $post_cr );
};
...
sub grab_rerun {
    ...
    elsif (Test2::Harness::UI::UUID::looks_like_uuid($rerun)) {
