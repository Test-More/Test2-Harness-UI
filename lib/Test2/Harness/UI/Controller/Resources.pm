 ...
 * If selecting a run, server, or global, go to the latest state for it, and progress as more come in (stopping when run is done if it is for a run)
 * If selecting an ID show just it, no stamp selector
 * If selecting both go to the one, but show the stamp selector
